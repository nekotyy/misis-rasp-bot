...
После каждой успешной загрузки `GroupCatalog` сохраняет список групп на диск
(`<папка с bot.db>/group_catalog_cache.json`, то есть `runtime/` в Docker). Если сайт расписания
недоступен — при запросе новой подписки, поиске или подборе источника для фото через OCR — каталог
подставляет этот снимок вместо живых данных, так что ранее виденные группы (то есть практически все
группы МИСиС) продолжают находиться и после падения сайта, и после перезапуска бота во время падения.
Не помогает только для группы, которую сайт не показывал вообще ни разу за всё время работы бота.
