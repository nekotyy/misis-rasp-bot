...
### Импорт расписания с фото (OCR)

- `OCR_ENABLED` — включение импорта расписания из фотографии.
- `OCR_TESSERACT_CMD` — путь к бинарнику `tesseract` (пусто = искать в `PATH`).
- `OCR_LANGUAGES` — языки распознавания, по умолчанию `rus+eng`.
- `OCR_PSM` — режим сегментации страницы Tesseract, по умолчанию `6`.
...
расписания, бот распознает ее и обновляет расписание так же, как если бы данные пришли с сайта.

Модули: `src/ocr_schedule.py` (распознавание, разбор, проверка) и `src/ocr_import.py` (подбор источника,
...
Распознанный текст превращается в обычный `ScheduleSnapshot` — тот же объект, что отдает парсер сайта.
...
Распознанный текст проходит несколько слоев очистки, потому что OCR ошибается предсказуемым образом:

- чинятся латинские двойники кириллицы (`Ky6aHeBa` -> `Кубанева`), цифры вместо букв и заглавные буквы,
  застрявшие внутри слова;
- выбрасываются шапки таблицы и заголовок страницы;
- строка пары разбирается по шаблону `номер | дисциплина | преподаватель | аудитория`,
  якорем служит шаблон ФИО `Фамилия И.О.`;
- строка пары без инициалов у преподавателя (`Консульт. Консультирующий 305/2`) разбирается по хвостовой
  аудитории плюс словарь, а в крайнем случае по сокращению с точкой;
...
- считается уверенность распознавания; ниже `OCR_MIN_CONFIDENCE` бот отдельно предупреждает.

Проверяются: читаемость даты и ее правдоподобность, уникальность и непрерывность номеров пар,
непустая дисциплина, формат ФИО и аудитории. Ошибки блокируют импорт, предупреждения — нет.
...
`OCR_ENGINE` выбирает движок. Их два, и разница между ними принципиальная.

- **`easyocr`** (по умолчанию) — сначала нейросетью находит рамки с текстом, потом читает каждую.
  Только он справляется с **фотографией экрана**: муар, рамки таблицы и съёмка под углом ему не мешают.
  Тяжёлый — тянет torch, образ вырастает примерно до 2.5 ГБ. В Docker входит вместе с моделями,
  локально ставится через `uv sync --extra ocr`.
- **`tesseract`** — лёгкий и быстрый, но без стадии детекции. Годится только для **чистых скриншотов**;
  на фотографии экрана не вытаскивает ничего. Остаётся запасным вариантом и подхватывается
  автоматически, если easyocr не установлен.

Замер на реальном фото расписания, снятом с монитора: Tesseract — 0 пар из 4 в любом режиме `PSM`
и с любой предобработкой; EasyOCR — распознал весь текст.

Для локального запуска на Windows Tesseract ставится отдельно
([сборка UB-Mannheim](https://github.com/UB-Mannheim/tesseract/wiki), нужен языковой пакет `rus`),
путь указывается в `OCR_TESSERACT_CMD`. Если ни один движок не доступен, кнопка импорта сообщит
об этом, а остальной бот продолжит работать как обычно.

### Сборка строк таблицы

Отдельная задача — понять, какая аудитория относится к какой паре. На снимке под углом правый край
таблицы уезжает вверх (на тестовом фото — на 80 пикселей), и сопоставлять ячейки по вертикали
бесполезно. Поэтому блоки раскладываются **по колонкам**: перспектива не меняет порядок ячеек
внутри колонки, так что n-я запись в колонке «Дисциплина» относится к той же паре, что n-я
в колонке «Ауд.». Колонки строятся отдельно для каждого дня, иначе пары уехали бы в чужие даты.
