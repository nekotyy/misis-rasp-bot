...
- `OCR_GEMINI_MODEL` — модель Gemini (пусто = модель по умолчанию для аккаунта).
...
подписаться на неё через `/group_setup` нельзя, пока сайт не подтвердит номер — бот прямо об этом
скажет. Как только `GroupCatalog` в следующий раз успешно сходит на сайт, запись без ID заменится
настоящей автоматически (это тот же самый механизм полной замены каталога).
...
прошлым снимком; часть групп может не резолвиться (например, ещё ни разу не было подписки на такую
группу, а сайт недоступен) — предпросмотр покажет, для скольких групп нашёлся источник, и подтверждение
применит только их, не блокируя весь импорт из-за нескольких нерезолвленных строк.
...
следуют два практических момента:

- куки нужно доставать из браузера после входа на gemini.google.com (DevTools -> Application/Storage
  -> Cookies). `__Secure-1PSIDTS` периодически протухает, но `gemini_webapi` сам обновляет его в
  фоне, пока жив процесс, и сохраняет по пути `GEMINI_COOKIE_PATH` (в Docker уже указывает в
  смонтированный `runtime/`, чтобы обновлённая кука не терялась при рестарте контейнера). Ручное
  обновление из браузера нужно, только если протухнет сам `__Secure-1PSID` (например, вышли из
  аккаунта или сменили пароль) — тогда распознавание начнёт падать с ошибкой авторизации;
